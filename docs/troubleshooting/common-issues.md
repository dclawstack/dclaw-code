...
# Check events
kubectl get events -n dclaw-code --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-code deployment/dclaw-code-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-code-db-credentials -n dclaw-code
   ```
