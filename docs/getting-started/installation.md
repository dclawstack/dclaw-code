...
2. Find **DClaw Code** in the app grid
3. Click **Install**
4. The DClaw Operator will provision:
   - Namespace: `dclaw-code`
...
spec:
  ...
  enabled: true
  frontend:
    image: ghcr.io/dclawstack/dclaw-code:latest
    replicas: 2
  backend:
    image: ghcr.io/dclawstack/dclaw-code-backend:latest
    replicas: 2
  database:
    enabled: true
    storage: 10Gi
...
kubectl get ingress -n dclaw-code
```
